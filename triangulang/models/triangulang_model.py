...
class TrianguLangModel(nn.Module):
    """
    TrianguLang: Geometry-Aware Semantic Consensus

    Architecture:
        1. SAM3 backbone + encoder (FROZEN) → encoder_memory
        2. DA3 (FROZEN) → depth → pointmaps
        3. GASA Decoder (TRAINABLE) → object queries
           - World-Space PE (replaces 2D PE)
           - Geometric attention bias
           - Text-conditioned queries
           - Presence token (predicts if object exists)
           - Box prompt encoding (geometry encoder)
        4. SAM3 seghead → masks

    Mask Selection Strategies:
        - 'confidence': Pick mask with highest mean logit (naive)
        - 'iou_match': Pick mask with best IoU to GT during training (SAM3's approach)
        - 'majority_vote': Aggregate top-k masks via soft voting
    """
    ...
    def forward(self, images, text_prompts, gt_masks=None,
        ...
        """Forward pass with GASA decoder.

        Args:
            images: [B, C, H, W] input images (or [B, N, C, H, W] if cross_view_mode=True)
            text_prompts: List[str] text prompts for each image (or flat list of K*B for multi-object)
            gt_masks: [B, H, W] optional ground truth masks for box prompt extraction
            box_prompts: [B, N_boxes, 4] optional pre-computed box prompts (for evaluation)
            box_labels: [B, N_boxes] optional box labels (1=positive, 0=negative)
            point_prompts: [B, N_points, 2] optional pre-computed point prompts (for evaluation)
            point_labels: [B, N_points] optional point labels (1=positive, 0=negative)
            gt_extrinsics: [B, 4, 4] optional GT camera extrinsics (camera-to-world)
                           If provided, pointmaps will be in world coordinates (for sheaf loss)
            gt_intrinsics: [B, 3, 3] optional GT camera intrinsics
            spatial_qualifier_idx: [B] optional spatial qualifier indices for spatial token conditioning
            intrinsics_orig_hw: tuple (H, W) original resolution at which gt_intrinsics are defined.
                               Required for correct focal/300 scaling when using gt_intrinsics.
            cached_depth: [B, 1, H, W] optional pre-computed depth from DA3 (bypasses DA3 for 2-4x speedup)
            da3_extrinsics: [B, 4, 4] optional DA3-NESTED estimated extrinsics (for world-frame GASA)
            da3_intrinsics: [B, 3, 3] optional DA3-NESTED estimated intrinsics

            cross_view_mode: bool, if True, dispatch to forward_multiview() for true cross-view attention.
                             This is required for DDP to properly synchronize gradients.
            num_texts: int, number of text prompts per batch item (K for multi-object, 1 for single)

        Returns:
            dict with pred_masks, pred_logits, depth, pointmaps, and presence_logit (if enabled)
        """
